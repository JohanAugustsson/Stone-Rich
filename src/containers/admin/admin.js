...
class admin extends Component {
    ...
    getAllProducts = (allProducts, qurrentId = '') => {
        ...
        let chosenProducts = list.map(obj => {
            return (
                (this.state.edit && this.state.editId === obj.id)
                ? <div key={obj.id} className='admin-article-object'>
                    <div className='change-content'>
                        ...
                        <div className='save-content'>
                            ...
                            <p className='admin-pointer-btn' onClick={() => console.log('spara')}>
                            ...
                            <div className='textAreaContainer'>
                                ...
                                <textarea onChange={(e) => console.log(e.target)} value={obj.productinfo} name="name" rows="8" cols="80"></textarea>
                                {
                                    (obj.numberinstore > 0)
                                        ...
                                        : <p className='notInStore'>slutsåld</p>
                                }
                            </div>
                        </div>
                ...
                : <div key={obj.id} className='admin-article-object'>
                    <div className='change-content'>
                        <h3>id: {obj.id}</h3>
                        <p className='admin-pointer-btn' onClick={() => this.switchToChange(obj.id)}>
                            <i className="far fa-edit"></i>
                            Ändra</p>
                    </div>
                    <div className='main-content'>
                        <div className='img-name-instore'>
                            <div className='img-container'>
                                <img src={(
                                        obj.img)
                                        ? obj.img
                                        : 'img/placeholder.png'} alt='product bild' title='productbild'/>
                            </div>
                            <div>
                                <p className='miniHeader'>Namn</p>
                                <p>{obj.name}</p>
    ...
    render() {
        ...
        return (<div className="admin-container">
            {
                (this.state.currentPage === 'allProducts')
                    ? <h1>Lager</h1>
                    : <h1>Lägg till produkt</h1>
            }
            <div className='admin-button-container'>
                <button onClick={() => this.updatePage('allProducts')} className={(
                        this.state.currentPage === 'allProducts')
                        ? 'admin-chosen-category'
                        : ''}>Produkter på lager</button>
                <button onClick={() => this.updatePage('addProducts')} className={(
                        this.state.currentPage === 'addProducts')
                        ? 'admin-chosen-category'
